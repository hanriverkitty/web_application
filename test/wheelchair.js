function wheelchair() {
    cursor();
    image(wheelchair_img, 0, 0, 1280, 720);
    glass = false;
    loadPixels();
    //손전등 원크기
    let rad = 300;
    wheelchair_img.loadPixels();
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            let pos = (x + y * width) * 4;
            let r = wheelchair_img.pixels[pos];
            let g = wheelchair_img.pixels[pos + 1];
            let b = wheelchair_img.pixels[pos + 2];
